...
async function ensureSupabaseModule() {
  const existing = resolveCreateClient()
  if (existing) return existing
  try {
    const mod = await import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm')
    const createFn = mod?.createClient || mod?.default?.createClient
...
export function initSupabase({ url, key } = {}) {
  ...
  function isJwt(k) {
    try { return typeof k === 'string' && /\w+\.\w+\.\w+/.test(k) } catch { return false }
  }
  if (!isJwt(supaKey)) {
    throw new Error('Chave inválida. Use a anon public key (JWT eyJ...) de Settings → API.')
  }

  // Persiste para próximos usos
  setStoredCreds(supaUrl, supaKey)

  const createClientFn = resolveCreateClient()
  if (!createClientFn) {
...
// Versão assíncrona com fallback para ESM caso a UMD não tenha carregado
export async function initSupabaseAsync({ url, key } = {}) {
  const fallbackUrl = (typeof window !== 'undefined' && window.SUPABASE_URL) || ''
  const fallbackKey = (typeof window !== 'undefined' && window.SUPABASE_ANON_KEY) || ''
  let supaUrl = url || getStoredCreds().url || fallbackUrl
  let supaKey = key || getStoredCreds().key || fallbackKey
  ...
  function isJwt(k) { try { return typeof k === 'string' && /\w+\.\w+\.\w+/.test(k) } catch { return false } }
  if (!isJwt(supaKey)) {
    throw new Error('Chave inválida. Use a anon public key (JWT eyJ...) de Settings → API.')
  }
  setStoredCreds(supaUrl, supaKey)
  const createClientFn = (resolveCreateClient() || await ensureSupabaseModule())
  if (!createClientFn) {
    throw new Error('Biblioteca Supabase não carregada')
  }
  supabaseClient = createClientFn(supaUrl, supaKey)
...
export async function saveData(state, createBackupManual = false) {
  if (!supabaseClient) {
    throw new Error('Supabase não inicializado. Chame initSupabase primeiro.')
  }
  const job = async () => {
    lastSavedState = state
    if (createBackupManual) {
  ...
  const next = saveQueue.then(job, job)
...
export function subscribeRealtime(onUpdate) {
  ...
  const ch = supabaseClient
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'app_data', filter: 'id=eq.global_state' },
      async () => {
        try {
          const state = await loadData()
          if (state && typeof onUpdate === 'function') onUpdate(state)
        } catch {}
      }
    )
  ch.subscribe()
...
if (typeof window !== 'undefined') {
  window.SupaSync = { initSupabase, initSupabaseAsync, loadData, saveData, createBackup, startAutoBackup, listBackups, restoreBackup, subscribeRealtime }
}
